...
async def on_member_join(user):
    ...
    if len(members_count) > MAX_MEMBERS_PER_SECOND:
        user.kick(reason="RAID DETECTED")
...
class Help(discord.ui.View):
    ...
    async def helpbtn(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id not in usertickets:
            ...
            interaction.user.guild.get_role(1105417181774233620): discord.PermissionOverwrite(view_channel=True)
            }
...
async def giveaway(interaction: discord.Interaction, item: str, time: int, winners: int):
    """Host a giveaway"""
    if interaction.guild.get_role(1099971435310628874) in interaction.user.roles or interaction.user.top_role > interaction.guild.get_role(1099971435310628874):
...
async def report(interaction: discord.Interaction, user: discord.Member, type: discord.app_commands.Choice[int], reason: str):
    ...
    overwrites = {
        interaction.guild.default_role: discord.PermissionOverwrite(view_channel = False),
        interaction.guild.get_role(1105417181774233620): discord.PermissionOverwrite(view_channel = True)
    }
...
async def on_message(message):
    ...
    if userxp[message.author.id] >= 100:
        if message.author.id in userxp:
            ...
            userxp[message.author.id] = 0
            await message.guild.get_channel(1105418899132010557).send(f"**{message.author} levelled up to {userlevel[message.author.id]}! Congratulations!**")
    if message.author.id in userlevel:
        if userlevel[message.author.id] == 5:
            await message.author.add_roles(message.guild.get_role(1105418995689082961))
    ...
    if any(bad_word in message.content.lower() for bad_word in bad_words):
        ...
        if warns[message.author.id] >= 5:
            await message.author.add_roles(message.author.guild.get_role(982758490953109594))
...
#ANTISPAM
    ...
    if message_counts[message.author] > 3 and not message.author.guild_permissions.manage_messages:
            ...
            if warns[message.author] >= 5:
                ...
                roletoremove = discord.utils.get(message.guild.roles, id=982758490953109594)
...
#TOKEN = "NzAyMDQzMTk3MTE5MDA0NzQy.Grzzk5.DDvEoW4p14tilS_jijJHKdSF3qPI9eZm4WvC1s" 
TOKEN = "MTAyNzIzNTM2OTIwMjI0MTYwNg.GV6lxf.2W208VeGmqUP95NAeIP2mwC5tgB_GzbRpsVpBg"
bot.run(TOKEN)